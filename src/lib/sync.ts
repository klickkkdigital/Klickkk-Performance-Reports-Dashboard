...
async function syncConnection(
  conn: { id: string; clientId: string; platform: Platform; accountId: string; accessToken: string; refreshToken: string | null },
  dateStart: string,
  dateEnd: string,
) {
  ...
  try {
    ...
    if (conn.platform === 'SHOPIFY') await syncShopify(conn.clientId, conn.accountId, token, dateStart, dateEnd)
